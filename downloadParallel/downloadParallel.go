...
func main() {
    res, _ := http.Head("http://192.168.1.25:8888/Fileshare_1.rar"); // 187 MB file of random numbers per line
    maps := res.Header
    ...
    for i := 0; i < limit ; i++ {
        ...
        go func(min int, max int, i int) {
            client := &http.Client {}
            req, _ := http.NewRequest("GET", "http://192.168.1.25:8888/Fileshare_1.rar", nil)  
            ...
            wg.Done()
            //ioutil.WriteFile("output.rar", []byte(string(body)), 0x777)
        }(min, max, i)
    }
    wg.Wait()
}
